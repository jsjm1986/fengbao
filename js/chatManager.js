class ChatManager {
    ...
    async getWuYunLiuQiAnalysis(birthYear, birthMonth, birthDay, calendarType) {
        const messages = [
            { role: 'system', content: WUYUN_LIUQI_PROMPT },
            { role: 'user', content: `请根据出生日期：${birthYear}年${birthMonth}月${birthDay}日（${calendarType === 'lunar' ? '农历' : '阳历'}），进行五运六气分析，并结合当前时节提供养生建议。` }
        ];
